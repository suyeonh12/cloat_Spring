   ...
   <section class="content board write">
		...
		<div class="inner">
			<!-- 새 글이면 NewsUpload, 수정이면 newsEdit -->  
		      <form action="${news != null ? 'updateNewsEdit' : 'NewsUpload'}"
		         method="post" enctype="multipart/form-data">
		         <ul class="write_ul">
		            <li>
		            	<span>ID</span> 
		            	<input type="text" name="admin_id" value="${mvo.id}" 
		            	class="ipt_tt" readonly>
		            </li>
		            <li>
		            	<span>제목</span> 
		            	<input type="text" name="news_title" class="ipt_tt"
		             	required value="${news != null ? news.news_title : ''}">
		            </li>
		            <li class="flex_dir_col">
		            	<span>내용</span> 
		            	<div class="post-form">
		            		<textarea name="news_content" id="summernote" required>
		            		${news != null ? news.news_content : ''}
		            		</textarea>
		            	</div>
		            </li>
		            <li>
		            	...
		            	<div class="is_file">
	            			...
	            			<input type="hidden" name="news_file" value="${news.news_file}" />
	            			<div class="pre_file_path"><p>${news.news_file}</p></div> <!-- 기존 파일 경로 -->                 
		               		</c:if>
		         ...
		         <c:if test="${news != null}">
		            <input type="hidden" name="news_idx" value="${news.news_idx}" />
		         </c:if>
		
		         <div class="bttn_wrap">
		         	<a class="bttn bttn_cancle" href="javascript:history.back();">취소</a>
		            <input class="bttn ipt_sbm" type="submit" value="${news != null ? '수정완료' : '작성완료'}">
		         </div>
		      </form>
		</div>	
		<script>
